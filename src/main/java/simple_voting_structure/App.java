...
public class App extends BaseAgent {
	...
	protected void setup() {
		...
		try {
			...
			votersName.forEach(voter -> {
				...
				logger.log(Level.INFO,
						String.format("%s CREATED AND STARTED NEW VOTER: %s ON CONTAINER %s", getLocalName(),
								voter, container.getName()));
			});
		} catch (Exception any) {
		...
		logger.log(Level.INFO,
				String.format("%s SENT START MESSAGE  TO %s", getLocalName(), votersName.get(votingStarter)));
	}
